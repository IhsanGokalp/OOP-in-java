...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		for(Marker eq: quakeMarkers) {
			String name = (String) eq.getProperty("country");
			if(eq.getProperty("country") == null) {
				oceanq++;
			}
			else {
				if(hm.containsKey(name)) {
					int counter = hm.get(name)+1;
					hm.put(name,counter);
				}
				else {
					hm.put(name,1);
		...
		System.out.println("Oceanquakes: "+ oceanq);
